 ...
 */
function parsePriceConstraint(text) {
...
function filterByPrice(products, { maxPrice = null, minPrice = null } = {}) {
...
function formatProductRecommendation(products) {
...
/** Грубое определение языка последнего сообщения: 'uk' | 'ru' | 'en'. */
...
function buildSystemPrompt(agent, { contextChunks, qaMatches, productMatches, settings, userText }) {
  ...
  // Строгие анти-галлюцинационные инструкции: отвечать ТОЛЬКО по контексту,
  // при отсутствии ответа — извиниться и предложить оставить контакт (fallback-сценарий).
  const fallbackHint = settings?.emailFallbackEnabled
    ? ' и предложи оставить контакт (email или телефон), чтобы с человеком связался специалист'
    : ' и предложи переформулировать вопрос или связаться с поддержкой';
  prompt +=
    `\n\nВАЖНЫЕ ПРАВИЛА:\n` +
    `1. Отвечай на вопрос пользователя, ОПИРАЯСЬ ТОЛЬКО на приведённый выше контекст (база знаний, Q&A, каталог).\n` +
    `2. Если ответа нет в контексте — НЕ ВЫДУМЫВАЙ факты, цены, характеристики или обещания. ` +
    `Вежливо извинись, что не располагаешь этой информацией${fallbackHint}.\n` +
    `3. Будь краток и по существу.`;

  if (settings?.autoLanguage) {
    const langName = LANG_NAMES[detectLanguage(userText)];
    if (langName) {
      prompt +=
        `\n4. ЯЗЫК ОТВЕТА: пиши ответ ИСКЛЮЧИТЕЛЬНО на ${langName} языке, даже если контекст, ` +
        `база знаний или товары написаны на другом языке. Никогда не переключай язык — это обязательное правило.`;
    } else {
      prompt += `\n4. Отвечай на том же языке, на котором написал пользователь.`;
    }
  }
...
function checkEscalation(userText, settings) {
