(function () {
  ...
  const TRANSLATIONS = {
    uk: {
      ...
      hero_title: 'Оживіть ваш інтернет-магазин<br>розумним <span class="grad">ШІ чат-ботом</span> за 5 хвилин',
      hero_desc: 'Налаштуйте логіку ШІ, оформіть чат-віджет під свій бренд та вбудуйте на сайт одним рядком коду. Без розробників та складних інтеграцій.',
      ...
      how_step3_desc: 'Один рядок на Horoshop, OpenCart, WordPress чи Tilda — і агент уже працює.',
      faq_eyebrow: 'FAQ',
      faq_title: 'Часті запитання про чат-боти для інтернет-магазинів',
      faq_q1: 'Що таке ШІ чат-бот для інтернет-магазину?',
      faq_a1: 'Це чат-віджет зі штучним інтелектом, який встановлюється на сайт магазину і спілкується з покупцями 24/7: відповідає на запитання про товари, доставку й оплату, рекомендує товари з каталогу та збирає контакти покупців. Emble навчається на базі знань саме вашого магазину, тому відповідає точно і не вигадує фактів.',
      faq_q2: 'Як встановити чат-віджет на сайт інтернет-магазину?',
      faq_a2: 'Достатньо вставити один рядок коду перед закриваючим тегом body — віджет запрацює на будь-якій платформі: Horoshop, OpenCart, WordPress (WooCommerce), Tilda, Shopify чи самописному сайті. Встановлення займає близько 5 хвилин і не потребує розробника.',
      faq_q3: 'Чи підлаштовується чат-бот під будь-яку мову?',
      faq_a3: 'Так. Бот автоматично визначає мову покупця та відповідає нею — українською, англійською, польською чи будь-якою іншою. Мову він розпізнає для кожного повідомлення, тому діалог може навіть перемикатися між мовами.',
      faq_q4: 'Що вміє ШІ-асистент для e-commerce?',
      faq_a4: 'Він відповідає на типові запитання про доставку, оплату та повернення, рекомендує товари з вашого каталогу з цінами й посиланнями, збирає ліди (email або телефон), проводить покупця сценаріями з кнопками та передає складні діалоги живому оператору.',
      faq_q5: 'Як навчити бота на даних мого магазину?',
      faq_a5: 'Завантажте файли (PDF, DOCX), додайте посилання на сторінки сайту, готові пари «запитання — відповідь» або імпортуйте каталог товарів. Бот проіндексує ці дані й відповідатиме на їх основі, а на запитання без відповіді запропонує залишити контакт.',
      faq_q6: 'Скільки коштує чат-бот Emble?',
      faq_a6: 'Доступ до панелі надається за індивідуальними умовами — логін і пароль ви отримуєте від нашої команди. Увійдіть у панель або зв\'яжіться з нами, щоб обговорити тариф під ваш обсяг повідомлень.',
      ...
      contacts_title: 'Залишились питання?<br>Напишіть нам',
      contacts_desc: 'Допоможемо з налаштуванням агента та підберемо рішення під ваше завдання. Відповідаємо протягом робочого дня.',
      contact_tg: 'Telegram',
    ...
    ru: {
      page_title: 'ИИ чат-бот для интернет-магазина | Emble — чат-виджет на сайт за 5 минут',
      ...
      hero_title: 'Оживите ваш интернет-магазин<br>умным <span class="grad">ИИ чат-ботом</span> за 5 минут',
      hero_desc: 'Настройте логику ИИ, оформите чат-виджет под свой бренд и встройте на сайт одной строкой кода. Без разработчиков и сложных интеграций.',
      ...
      how_step3_desc: 'Одна строка на Horoshop, OpenCart, WordPress или Tilda — и агент уже работает.',
      faq_eyebrow: 'FAQ',
      faq_title: 'Частые вопросы о чат-ботах для интернет-магазинов',
      faq_q1: 'Что такое ИИ чат-бот для интернет-магазина?',
      faq_a1: 'Это чат-виджет с искусственным интеллектом, который устанавливается на сайт магазина и общается с покупателями 24/7: отвечает на вопросы о товарах, доставке и оплате, рекомендует товары из каталога и собирает контакты покупателей. Emble обучается на базе знаний именно вашего магазина, поэтому отвечает точно и не выдумывает фактов.',
      faq_q2: 'Как установить чат-виджет на сайт интернет-магазина?',
      faq_a2: 'Достаточно вставить одну строку кода перед закрывающим тегом body — виджет заработает на любой платформе: Horoshop, OpenCart, WordPress (WooCommerce), Tilda, Shopify или самописном сайте. Установка занимает около 5 минут и не требует разработчика.',
      faq_q3: 'Подстраивается ли чат-бот под любой язык?',
      faq_a3: 'Да. Бот автоматически определяет язык покупателя и отвечает на нём — на украинском, английском, польском или любом другом. Язык он распознаёт для каждого сообщения, поэтому диалог может даже переключаться между языками.',
      faq_q4: 'Что умеет ИИ-ассистент для e-commerce?',
      faq_a4: 'Он отвечает на типовые вопросы о доставке, оплате и возвратах, рекомендует товары из вашего каталога с ценами и ссылками, собирает лиды (email или телефон), ведёт покупателя по сценариям с кнопками и передаёт сложные диалоги живому оператору.',
      faq_q5: 'Как обучить бота на данных моего магазина?',
      faq_a5: 'Загрузите файлы (PDF, DOCX), добавьте ссылки на страницы сайта, готовые пары «вопрос — ответ» или импортируйте каталог товаров. Бот проиндексирует эти данные и будет отвечать на их основе, а на вопрос без ответа предложит оставить контакт.',
      faq_q6: 'Сколько стоит чат-бот Emble?',
      faq_a6: 'Доступ к панели предоставляется на индивидуальных условиях — логин и пароль вы получаете от нашей команды. Войдите в панель или свяжитесь с нами, чтобы обсудить тариф под ваш объём сообщений.',
      ...
      contacts_title: 'Остались вопросы?<br>Напишите нам',
      contacts_desc: 'Поможем с настройкой агента и подберём решение под вашу задачу. Отвечаем в течение рабочего дня.',
      contact_tg: 'Telegram',
      contact_phone: 'Телефон',
      contact_email: 'Почта',
